  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // メール確認が有効なプロジェクト設定だと session が作られないため、
        // ここで "/" に遷移すると結局 /login に戻ってきて「登録できてない」ように見える。
        if (!data.session) {
          setMessage(
            "確認メールを送信しました。メール内のリンクで有効化してからログインしてください。"
          );
          setIsLogin(true);
          return;
        }

        // 即時ログインできる設定なら登録成功 → トップページへ
        router.push("/");
        router.refresh();
      }
